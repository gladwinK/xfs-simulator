  ...
  Implementation Details:
   ...
   - disk_init(size_t): Allocates the memory buffer and initializes to zeros
  ...
  Key Structures:
   ...
   - xfs_inode_t: Contains file metadata including extent list
  ...
  XFS divides the filesystem into multiple Allocation Groups to enable concurrent access and improve scalability.
  ...
  5.1 Extent-Based Storage
   - `xfs_extent_t`: Maps logical file offsets to physical disk blocks
   - Extent Management: Maintains ordered list of extents for each inode
  ...
  7.1 Thread Safety Model
   ...
   - Journal Serialization: Log queue operations use mutex for thread safety

  7.2 Deadlock Prevention
   - Lock Ordering: AG locks acquired in consistent order
   - No Nested Locks: Prevents circular dependency issues
   - Timeout Handling: Proper error handling for lock acquisition failures
  ...
  The simulation provides a realistic, interactive demonstration of how XFS handles the complex challenges of concurrent
  access, metadata consistency, and performance optimization in a user-space environment. Each component works in concert
   to provide a faithful simulation of XFS behavior while remaining comprehensible and educational.
